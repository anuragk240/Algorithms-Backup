...
public class SuffixTreeFromArray {
    ...
    public class Edge {
        ...
        Edge(int node, int start, int end, int depth, int parentNode) {
            this.node = node;
            this.start = start;
            this.end = end;
			this.depth = depth;
    ...
    Map<Integer, List<Edge>> SuffixTreeFromSuffixArray(
		...
		int totalNodes = 0, currentNode = 0;
		List<Edge> root = new ArrayList<Edge>();
		Edge temp = new Edge(++totalNodes, suffixArray[0], text.length(), text.length() - suffixArray[0], 0);
		root.add(temp);
		currentNode = totalNodes;
		
		for(int i = 0; i < suffixArray.length; ++i){
			while(lcp[i] < temp.depth){
